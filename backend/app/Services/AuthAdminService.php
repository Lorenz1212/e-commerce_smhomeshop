...
class AuthAdminService
{
    ...
    {
        $key = 'core_login_attempts:' . strtolower($data['email']) . '|' . request()->ip();
        ...
        Auth::guard('core')->setUser($user);

        $token = $user->createToken('Core Token: ' . $user->user_info->account_no)->accessToken;
    ...
    public function changePassword($request){
        ...
        if(!$user){
            throw ValidationException::withMessages([_lang_message('not_found',['item'=>'User'])]);
        }

        if (!Hash::check($request->current_password, $user->password)) {
            throw ValidationException::withMessages([_lang_message('password_incorrect',['item'=>'Current password'])]);
        }
