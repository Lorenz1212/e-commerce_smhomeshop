...
import { OnlineOrderTable } from './components/tables/OnlineOrderTable'
import { useOrder } from './core/_request'

const OrderList: FC<{title:string}> = ({title}) => {

  const table = useTableHook<OnlineOrderListModel>('/admin/order/online/list')  
  ...
  const handleReceiptDetails = async  (order_id: string) => {
    const res = await viewOrderDetails(order_id)
    if (!res) return;
    setModalState({
      visible: true,
      title: 'View Receipt',
